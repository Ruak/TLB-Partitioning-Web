#include "wrapper.h"

#include <string.h>

static const uint8_t sbox[256] = {
    
};

static const uint8_t rsbox[256] = {
    
};

static const uint8_t rcon[11] = {
    0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36
};

static uint8_t xtime(uint8_t x) {
    return (uint8_t)((x << 1) ^ (((x >> 7) & 1) * 0x1b));
}

static uint8_t multiply(uint8_t x, uint8_t y) {
    uint8_t result = 0;
    while (y) {
        if (y & 1) result ^= x;
        x = xtime(x);
        y >>= 1;
    }
    return result;
}

static void key_expansion(uint8_t *round_key, const uint8_t *key) {
    memcpy(round_key, key, 16);
    uint8_t temp[4];
    int bytes = 16;
    int rcon_idx = 1;

    while (bytes < AES128_ROUND_KEYS) {
        for (int i = 0; i < 4; i++) temp[i] = round_key[bytes - 4 + i];
        if (bytes % 16 == 0) {
            uint8_t t = temp[0];
            temp[0] = sbox[temp[1]] ^ rcon[rcon_idx++];
            temp[1] = sbox[temp[2]];
            temp[2] = sbox[temp[3]];
            temp[3] = sbox[t];
        }
        for (int i = 0; i < 4; i++) {
            round_key[bytes] = round_key[bytes - 16] ^ temp[i];
            bytes++;
        }
    }
}

static void add_round_key(uint8_t state[16], const uint8_t *round_key) {
    for (int i = 0; i < 16; i++) state[i] ^= round_key[i];
}

static void sub_bytes(uint8_t state[16]) {
    for (int i = 0; i < 16; i++) state[i] = sbox[state[i]];
}

static void inv_sub_bytes(uint8_t state[16]) {
    for (int i = 0; i < 16; i++) state[i] = rsbox[state[i]];
}

static void shift_rows(uint8_t state[16]) {
    uint8_t t;
    t = state[1]; state[1] = state[5]; state[5] = state[9]; state[9] = state[13]; state[13] = t;
    t = state[2]; state[2] = state[10]; state[10] = t; t = state[6]; state[6] = state[14]; state[14] = t;
    t = state[3]; state[3] = state[15]; state[15] = state[11]; state[11] = state[7]; state[7] = t;
}

static void inv_shift_rows(uint8_t state[16]) {
    uint8_t t;
    t = state[13]; state[13] = state[9]; state[9] = state[5]; state[5] = state[1]; state[1] = t;
    t = state[2]; state[2] = state[10]; state[10] = t; t = state[6]; state[6] = state[14]; state[14] = t;
    t = state[3]; state[3] = state[7]; state[7] = state[11]; state[11] = state[15]; state[15] = t;
}

static void mix_columns(uint8_t state[16]) {
    for (int i = 0; i < 4; i++) {
        uint8_t *col = &state[i * 4];
        uint8_t a = col[0], b = col[1], c = col[2], d = col[3];
        col[0] = multiply(a, 2) ^ multiply(b, 3) ^ c ^ d;
        col[1] = a ^ multiply(b, 2) ^ multiply(c, 3) ^ d;
        col[2] = a ^ b ^ multiply(c, 2) ^ multiply(d, 3);
        col[3] = multiply(a, 3) ^ b ^ c ^ multiply(d, 2);
    }
}

static void inv_mix_columns(uint8_t state[16]) {
    for (int i = 0; i < 4; i++) {
        uint8_t *col = &state[i * 4];
        uint8_t a = col[0], b = col[1], c = col[2], d = col[3];
        col[0] = multiply(a, 14) ^ multiply(b, 11) ^ multiply(c, 13) ^ multiply(d, 9);
        col[1] = multiply(a, 9) ^ multiply(b, 14) ^ multiply(c, 11) ^ multiply(d, 13);
        col[2] = multiply(a, 13) ^ multiply(b, 9) ^ multiply(c, 14) ^ multiply(d, 11);
        col[3] = multiply(a, 11) ^ multiply(b, 13) ^ multiply(c, 9) ^ multiply(d, 14);
    }
}

void wp_set_encrypt_key128(void *rdkey, const uint8_t *key) {
    CRYPT_AES_Key *ctx = (CRYPT_AES_Key *)rdkey;
    key_expansion(ctx->round_key, key);
}

void wp_set_decrypt_key128(void *rdkey, const uint8_t *key) {
    wp_set_encrypt_key128(rdkey, key);
}

void wp_encrypt_block(uint8_t *input, uint8_t *output, void *rdkey) {
    CRYPT_AES_Key *ctx = (CRYPT_AES_Key *)rdkey;
    uint8_t state[16];
    memcpy(state, input, 16);

    add_round_key(state, ctx->round_key);
    for (int round = 1; round < 10; round++) {
        sub_bytes(state);
        shift_rows(state);
        mix_columns(state);
        add_round_key(state, ctx->round_key + round * 16);
    }
    sub_bytes(state);
    shift_rows(state);
    add_round_key(state, ctx->round_key + 160);
    memcpy(output, state, 16);
}

void wp_decrypt_block(uint8_t *input, uint8_t *output, void *rdkey) {
    CRYPT_AES_Key *ctx = (CRYPT_AES_Key *)rdkey;
    uint8_t state[16];
    memcpy(state, input, 16);

    add_round_key(state, ctx->round_key + 160);
    for (int round = 9; round > 0; round--) {
        inv_shift_rows(state);
        inv_sub_bytes(state);
        add_round_key(state, ctx->round_key + round * 16);
        inv_mix_columns(state);
    }
    inv_shift_rows(state);
    inv_sub_bytes(state);
    add_round_key(state, ctx->round_key);
    memcpy(output, state, 16);
}
